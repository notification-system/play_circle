...
public class EmailTemplateService {
    ...
    @Autowired
    public EmailTemplateService(EmailTemplateRepository emailTemplateRepository, Producer producer,
    ...
    public EmailTemplate newEmailTemplate(EmailTemplate emailTemplate, String inputSystemId, String email) {
        InputSystem inputSystem = getInputSystemForTemplate(inputSystemId, email);
        if (emailTemplateRepository.findEmailTemplateByEmailTemplateNameAndInputSystemId(emailTemplate.getEmailTemplateName(), inputSystemId) != null) {
    ...
    public EmailTemplate postEmailTemplate(String inputSystemId, String emailTemplateId,
        ...
        if (!emailTemplate.getEmailFromProvider().equals(EmailFromProvider.DEFAULT) && auth) {
            throw new GeneralException(MessageError.USERNAME_AND_PASSWORD_ARE_REQUIRED_FOR_NON_DEFAULT.getMessageText());
        ...
        if (text.contains("#{") || text.contains("}#")) {
            throw new GeneralException(MessageError.NOT_ENOUGH_PARAMETERS_FOR_PLACEHOLDERS.getMessageText());
        }
