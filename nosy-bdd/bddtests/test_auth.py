...
def test_auth_get_token():
    r = requests.post(
        url=get_token_url, data=json.dumps(api_user_create), headers=headers
    )
...
def test_get_status():
    ...
    r = requests.post(
        url=status_token_url, data=json.dumps(json_token), headers=headers
    )
    assert r.text == "true"
...
def test_email_admin_inputsystems_create():
    ...
    headers_auth = {"Content-type": "application/json", "Authorization": json_bearer}
    r = requests.post(
        url=create_inputsystemdto_url,
        data=json.dumps(create_inputsystemdto),
